...
contract Chess is TurnBasedGame, Auth {
    ...
    */
    function moveFromState(bytes32 gameId, int8[128] memory state, uint256 fromIndex,
        ...
        if ((state[8] * int8(int(128)) + state[9]) < (gameStates[gameId].fields[8] * int8(int(128)) + gameStates[gameId].fields[9])) {
            revert();
        }
    ...
    function claimWin(bytes32 gameId) notEnded(gameId) public override {
        ...
        uint256 kingIndex = uint256(int256(gameStates[gameId].getOwnKing(otherPlayerColor)));
