...
library ChessLogic {
     ...
     * Because there are no arrays in Solidity, we use byte literals that
     ...
     */
    function validateMove(State storage self, uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 toFigure, int8 movingPlayerColor) public returns (bool) {
        ...
        if (abs(fromFigure) == uint(int(Pieces(Piece.WHITE_BISHOP))) ||
            abs(fromFigure) == uint(int(Pieces(Piece.WHITE_QUEEN))) ||
            abs(fromFigure) == uint(int(Pieces(Piece.WHITE_ROOK)))) {

            // Bishop can only walk diagonally, Rook only non-diagonally
            if (!isDiagonal && abs(fromFigure) == uint(int(Pieces(Piece.WHITE_BISHOP))) ||
                isDiagonal && abs(fromFigure) == uint(int(Pieces(Piece.WHITE_ROOK)))) {
                return false;
            }
        ...
        // Pawns
        if (abs(fromFigure) == uint(int(Pieces(Piece.WHITE_PAWN)))) {
    ...
    function checkForCheck(State storage self, uint256 kingIndex, int8 currentPlayerColor) internal returns (bool) {
        ...
        for (uint move = 0; move < 8; move ++){
            ...
            int8 currentMoveIndex = int8(uint8(kingIndex)) + int8(uint8(knightMoves[move])) - 64;

            // if inside the board
            if (uint(int(currentMoveIndex)) & 0x88 == 0){
    ...
    function makeMove(State storage self, uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 toFigure) internal {
        ...
        if (fromFigure == Pieces(Piece.BLACK_KING)) {
            ...
            setFlag(self, Flag.BLACK_KING_POS, int8(int(toIndex)));
            // Castling
        ...
        if (fromFigure == Pieces(Piece.WHITE_KING)) {
            // Update position flag
            setFlag(self, Flag.WHITE_KING_POS, int8(int(toIndex)));
            // Castling
